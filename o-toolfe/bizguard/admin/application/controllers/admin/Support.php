...
class Support extends MY_Controller {
    ...
    public function chat( $id = 0 )
    {
        ...
        if ( empty( $chat ) )
        {
            error_redirect( 'chat_not_found', 'admin/chats/all' );
        }
        ...
        $data['data']['chat_envato_response'] = $this->Support_model->chat_envato_response( $chat->id );
        $data['data']['chat'] = $chat;
    ...
    private function tickets( $type = 'all' )
    {
        ...
        if ( $type === 'all' )
        {
            ...
            $data['title'] = lang( 'all_tickets' );
				...
				else if ( $type === 'my_departments' )
        {
            $config['base_url'] = env_url( 'admin/tickets/my_departments' );
            $data['title'] = lang( 'my_departments_tickets' );
						$to_fetch['my_departments'] = true;
            $to_count['my_departments'] = true;
        }
        else if ( $type === 'opened' )
        {
    ...
    public function ticket( $id = 0 )
    {
        ...
        if ( empty( $ticket ) )
        {
            error_redirect( 'ticket_not_found', 'admin/tickets/all' );
        }
        ...
        $data['data']['notes'] = $this->Support_model->ticket_notes( ['ticket_id' => $id] );
        ...
        $data['data']['ticket_envato_response'] = $this->Support_model->ticket_envato_response( $ticket->id );
