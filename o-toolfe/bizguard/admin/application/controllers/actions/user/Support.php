...
class Support extends MY_Controller {
    ...
    /**
		...
		{
        if ( db_config( 'module_tickets' ) == 0 )
        {
            r_error( 'temp_disabled' );
        }
        
        if ( db_config( 'sp_allow_ticket_feedback_delete' ) == 0 )
        {
            r_error( 'invalid_req' );
        }
        ...
        else
        {
            ...
            $data = $this->Support_model->ticket( $ticket_id, $this->user_id );
				...
				if ( $this->Support_model->update_ticket( $to_update, $ticket_id ) )
				{
						log_ticket_activity( 'ticket_fb_deleted_user', $ticket_id );
						
						r_s_jump( $url, 'ticket_fb_deleted' );
				}
		...
		/**
		 * Share Ticket Feedback Input Handling.
		 *
		 * @return  void
		 * @version 2.0
		 */
        ...
        if ( db_config( 'module_tickets' ) == 0 )
        {
            r_error_gr( 'temp_disabled' );
        }
        
				if ( ! isset( $_POST['feedback_type'] ) )
				{
						r_error_gr( 'no_feedback' );
				}
        ...
        else
        {
            ...
            $data = $this->Support_model->ticket( $ticket_id, $this->user_id );
				...
				if ( $data->status != 0 ) r_error_gr( 'invalid_req' );
				
				$to_update = [
            'feedback_type' => intval( post( 'feedback_type' ) ),
            'feedback' => do_secure( post( 'feedback' ) )
				];
            ...
            {
                send_feedback_notification( $data->assigned_to, $ticket_id );
            }
                
						log_ticket_activity( 'ticket_fb_shared', $ticket_id );
						
						r_s_jump( $url, 'ticket_fb_shared' );
				}
    ...
    public function create_chat()
    {
        ...
        if ( $this->form_validation->run( 'create_chat' ) )
        {
            ...
            if ( db_config( 'envato_ask_purchase_code_chat' ) == 1 )
            {
                $envato_purchase_code = do_secure( post( 'envato_purchase_code' ) );
                
                $envato_response = $this->check_envato_purchase_code( $envato_purchase_code, 'chat' );
            }
            ...
            if ( ! empty( $id ) )
            {
                if ( ! empty( $envato_purchase_code ) && ! empty( $envato_response ) )
                {
                    $data_envato = [
                        'envato_purchase_code' => $envato_purchase_code,
                        'envato_purchase_code_response' => json_encode( $envato_response ),
                        'chat_id' => $id
                    ];

                    $this->Support_model->add_chat_envato_response( $data_envato );
                }
     ...
     *
     * @param   string $envato_purchase_code
     * @param   string $for ticket|chat
     * @return  object
     * @version 2.0
     */
    private function check_envato_purchase_code( $envato_purchase_code, $for )
    {
        if ( empty( $envato_purchase_code ) )
        {
            r_error( 'invalid_req' );
        }
        
        $this->load->library( 'ZEnvato' );
        
        $envato_response = $this->zenvato->verify_purchase( $envato_purchase_code );
        
        if ( $envato_response == 'no_token' )
        {
            r_error_gr( 'envato_no_token_passed' );
        }
        else
        {
            if ( $envato_response === false ) r_error_gr( 'went_wrong_epc' );
            
            if ( is_object( $envato_response ) )
            {
                if ( db_config( "envato_se_create_{$for}" ) == 0 && true )
                {
                    $url = db_config( 'envato_support_policy_url' );
                    $date = mb_substr( $envato_response->supported_until, 0, 10 );
                    $site_date = get_site_date( 'Y-m-d' );
                    
                    if ( $site_date > $date )
                    {
                        d_r_error_gr( sprintf( err_lang( 'item_support_expired' ), $date, $url ) );
                    }
                }
            }
            else if ( $envato_response == -1 || $envato_response == 0 )
            {
                r_error_gr( 'invalid_pc' );
            }
        }
        
        return $envato_response;
    }
    ...
    public function create_ticket()
    {
        ...
        if ( $this->form_validation->run( $form_validation_key ) )
        {
            ...
            if ( db_config( 'envato_ask_purchase_code_ticket' ) == 1 )
            {
                $envato_purchase_code = do_secure( post( 'envato_purchase_code' ) );
                
                $envato_response = $this->check_envato_purchase_code( $envato_purchase_code, 'ticket' );
            }
            ...
            if ( ! empty( $id ) )
            {
                if ( ! empty( $envato_purchase_code ) && ! empty( $envato_response ) )
                {
                    ...
                    $this->Support_model->add_ticket_envato_response( $data_envato );
                }
