...
class Users extends MY_Controller {
    ...
    public function new_user()
    {
        if ( $this->form_validation->run( 'new_user' ) )
        {
            ...
            $plane_mon = intval(post('plane_mon')); // Get the number of months from POST parameter

            if ($plane_mon == 0) {
                $days_to_add = 30; // Add 30 days if plane_mon is 0
            } elseif ($plane_mon == 1) {
                $days_to_add = 30 * 12; // Add 360 days if plane_mon is 1
            } else {
                // Handle other cases if needed, for now, let's default to 30 days
                $days_to_add = 30;
            }

            $plane_expire = date('Y-m-d', strtotime('+' . $days_to_add . ' days'));
            ...
            $data = [
                ...
                'is_verified' => 1,
                'user_plane' => post( 'user_plane' ),
                'plane_mon' => post( 'plane_mon' ),
                'plane_expire' => $plane_expire,
                'registered_month_year' => date( 'Y-m-d' ),
                'registered_at' => time()
            ];
