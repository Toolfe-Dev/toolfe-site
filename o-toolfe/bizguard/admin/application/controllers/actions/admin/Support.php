...
class Support extends MY_Controller {
     ...
     * @version 2.1
     */
    public function change_chat_department()
    {
        ...
        $department = $this->Support_model->department( $department );
        
        if ( empty( $department ) ) r_error( 'invalid_req' );
        
				$to_update = ['department_id' => $department->id];
				
				if ( $this->Support_model->update_chat( $to_update, $id, 'force' ) )
				{
            inform_department_users( $department, $id, 'chat' );
				
						r_s_jump( "admin/chats/chat/{$id}", 'department_changed' );
				}
     ...
     * @version 2.1
     */
    public function change_ticket_department()
    {
        ...
        if ( empty( $department ) ) r_error( 'invalid_req' );
				...
				if ( $this->Support_model->update_ticket( $to_update, $id, 'force' ) )
				{
            inform_department_users( $department, $id );
            
						$my_id = $this->zuser->get( 'id' );
						
						log_ticket_activity( 'ticket_department_changed', $id, $my_id );
				
						r_s_jump( "admin/tickets/ticket/{$id}", 'department_changed' );
				}
    ...
    public function change_ticket_priority()
    {
        ...
        $priority = do_secure( post( 'priority' ) );
        ...
        if ( ! in_array( $priority, ['low', 'medium', 'high'] ) )
        {
            r_error( 'invalid_req' );
				...
				if ( $this->Support_model->update_ticket( $to_update, $id, 'force' ) )
				{
						...
						log_ticket_activity( 'ticket_priority_changed', $id, $my_id );
				
						r_s_jump( "admin/tickets/ticket/{$id}", 'priority_changed' );
				}
     ...
     */
		...
		{
				check_action_authorization( 'tickets' );
				check_action_authorization( 'delete_ticket_feedback' );
				...
				$to_update = ['feedback_type' => null, 'feedback' => ''];
				
				if ( $this->Support_model->update_ticket( $to_update, $id, false ) )
				{
						$my_id = $this->zuser->get( 'id' );
						
						log_ticket_activity( 'ticket_fb_deleted', $id, $my_id );
				
						r_s_jump( "admin/tickets/ticket/{$id}", 'ticket_fb_deleted' );
				}
    ...
    public function add_ticket_note()
    {
        ...
        if ( $this->form_validation->run( 'support_note' ) )
        {
            $user_id = $this->zuser->get( 'id' );
            
            $data = [
                'note' => do_secure( post( 'note' ), true ),
                'user_id' => $user_id,
                'ticket_id' => $ticket->id,
                'created_at' => time()
            ];
            ...
            $id = $this->Support_model->add_ticket_note( $data );
            
            if ( ! empty( $id ) )
            {
                log_ticket_activity( 'ticket_note_added', $ticket->id, $user_id );
                
                r_s_jump( "admin/tickets/ticket/{$ticket->id}", 'note_added' );
            }
            
            r_error( 'went_wrong' );
        }
        
        d_r_error( form_error( 'note' ) );
    }
    ...
    public function delete_ticket_note()
    {
        ...
        $note = $this->Support_model->ticket_note( $id );
        
        if ( empty( $note ) ) r_error( 'invalid_req' );
        
        $attachment = $note->attachment;
        
        if ( $this->Support_model->delete_ticket_note( $id ) )
        {
            log_ticket_activity( 'ticket_note_deleted', $note->ticket_id, $this->zuser->get( 'id' ) );
            ...
            r_s_jump( "admin/tickets/ticket/{$note->ticket_id}", 'note_deleted' );
        }
    ...
    public function delete_chat()
    {
        ...
        if ( $this->Support_model->delete_chat( $id ) )
        {
            ...
            $this->Support_model->delete_chat_envato_response( $id );
    ...
    public function add_faq()
    {
        ...
        if ( $this->form_validation->run( 'faq' ) )
        {
            $data = [
                ...
                'logged_in_only' => only_binary( post( 'logged_in_only' ) ),
                'created_at' => time()
            ];
        ...
        if ( $this->form_validation->run( 'add_reply' ) )
        {
            ...
            if ( ! empty( $id ) )
            {
                ...
                if ( ! empty( $_FILES ) )
                {
                    ...
                    $attachments_data['ticket_reply_id'] = $id;
                    
                    foreach ( $_FILES as $key => $file )
                    {
                        if ( ! empty( $_FILES[$key]['tmp_name'] ) )
                        {
                            $file = $this->zfiles->upload_attachment( $key );
                            
                            $attachments_data['attachment_name'] = do_secure( $file['client_name'] );
                            $attachments_data['attachment'] = $file['file_name'];
                            
                            $this->Support_model->save_ticket_reply_attachment( $attachments_data );
                        }
                    }
                }
    ...
    {
        check_action_authorization( 'tickets' );
				check_action_authorization( 'edit_ticket_reply' );
    ...
    public function update_ticket_reply( $ticket_id = 0 )
    {
        ...
        if ( $this->form_validation->run( 'update_ticket_reply' ) )
        {
            ...
            if ( $this->Support_model->update_reply( $data, $id ) || $updated )
            {
                ...
                if ( ! empty( $_FILES ) )
                {
                    ...
                    foreach ( $_FILES as $key => $file )
                    {
                        if ( ! empty( $_FILES[$key]['tmp_name'] ) )
                        {
                            ...
                            $this->Support_model->save_ticket_attachment( $attachments_data );
                        }
                    }
                }
    ...
    {
        check_action_authorization( 'tickets' );
				check_action_authorization( 'delete_ticket_reply' );
        ...
        $reply_attachments = get_ticket_reply_attachments( $id );
        ...
        if ( $this->Support_model->delete_ticket_reply( $id ) )
        {
            ...
            if ( ! empty( $reply_attachments ) )
            {
                ...
                {
                    $this->zfiles->delete_image_file( 'attachments', $reply_attachment->attachment );
                }
                
                $this->Support_model->delete_ticket_reply_attachments( $id );
            }
    ...
    {
        check_action_authorization( 'tickets' );
        check_action_authorization( 'delete_ticket' );
        ...
        if ( $this->Support_model->delete_ticket( $id ) )
        {
            ...
            if ( ! empty( $replies ) )
            {
                ...
                foreach ( $replies as $reply )
                {
                    ...
                    if ( ! empty( $reply_attachments ) )
                    {
                        ...
                        $this->Support_model->delete_ticket_reply_attachments( $reply->id );
                    }
                }
            }
            
            $notes = $this->Support_model->ticket_notes( ['ticket_id' => $ticket->id] );
            
            if ( ! empty( $notes ) )
            {
                // Delete the ticket notes attachment(s) ( if attached ):
            ...
            {
                $ticket_attachments = get_ticket_attachments( $id );
                
                if ( ! empty( $ticket_attachments ) )
                {
                    foreach ( $ticket_attachments as $ticket_attachment )
                    {
                        $this->zfiles->delete_image_file( 'attachments', $ticket_attachment->attachment );
                    }
                    
                    $this->Support_model->delete_ticket_attachments( $id );
                }
            }
            ...
            $this->Support_model->delete_ticket_notes( $id );
            $this->Support_model->delete_ticket_envato_response( $id );
