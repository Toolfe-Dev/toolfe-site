      ...
      <div class="col-sm-12">
        ...
        <div class="card">
          <div class="card-header d-flex align-items-center">
            <h3 class="card-title"><?php echo lang( 'canned_replies' ); ?></h3>
            <div class="ml-auto">
              <button class="btn btn-success text-sm" data-toggle="modal" data-target="#add-canned-reply">
                <i class="fas fa-plus-circle mr-2"></i> <?php echo lang( 'add_canned_reply' ); ?>
              </button>
            </div>
            <!-- /.card-tools -->
          </div>
            ...
            <div class="table-responsive">
              <table class="custom-table z-table table table-striped text-nowrap table-valign-middle mb-0">
                ...
                <tbody class="records-tbody text-sm">
                  <?php
                  if ( ! empty( $canned_replies ) )
                  {
                    foreach ( $canned_replies as $canned_reply ) {
                      $id = $canned_reply->id; ?>
                      <tr id="record-<?php echo html_escape( $id ); ?>">
                        <td><?php echo html_escape( $id ); ?></td>
                        <td><?php echo html_escape( $canned_reply->subject ); ?></td>
                        <td>
                          <?php echo html_escape( short_text( $canned_reply->message ) ); ?>
                          
                          <?php if ( is_increased_short_text( $canned_reply->message ) ) { ?>
                        ...
                        <td class="text-right"><?php manage_updated_at( html_escape( $canned_reply->updated_at ) ); ?></td>
                        <td class="text-right"><?php echo get_date_time_by_timezone( html_escape( $canned_reply->created_at ) ); ?></td>
                        <td class="text-right">
                          <div class="btn-group">
