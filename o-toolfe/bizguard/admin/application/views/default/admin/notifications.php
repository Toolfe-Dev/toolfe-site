                  ...
                  if ( ! empty( $notifications ) )
                  {
                    foreach ( $notifications as $notify ) {
                      ...
                      <tr class="<?php echo ( $notify->is_read == 0 ) ? 'bg-light' : ''; ?>">
                        ...
                        <td class="text-right">
                          <button class="btn btn-link text-danger tool" data-id="<?php echo html_escape( $notify->id ); ?>" data-toggle="modal" data-target="#delete"><i class="fas fa-trash tool-c"></i></button>
...
<?php load_modals( ['admin/mark_all_as_read', 'admin/delete_notification'] ); ?>