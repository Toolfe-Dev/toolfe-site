...
class Support_model extends MY_Model {
    ...
    /**
     * Ticket Envato Response
     *
     * @param   integer $ticket_id
     * @return  object
     * @version 2.0
     */
    public function ticket_envato_response( $ticket_id )
    {
        $data['where'] = ['ticket_id' => $ticket_id];
        $data['table'] = 'tickets_envato_responses';
        
        return $this->get_one( $data );
    }
    
    /**
     * Add Ticket Envato Response
     *
     * @param   array $data
     * @return  integer
     * @version 2.0
     */
    public function add_ticket_envato_response( $data )
    {
        return $this->add( $data, 'tickets_envato_responses' );
    }
    
    /**
     * Delete Ticket Envato Response
     *
     * @param   integer $ticket_id
     * @return  boolean
     * @version 2.0
     */
    public function delete_ticket_envato_response( $ticket_id )
    {
        $data['where']['ticket_id'] = $ticket_id;
        $data['table'] = 'tickets_envato_responses';
    ...
    /**
     * Chat Envato Response
     *
     * @param   integer $chat_id
     * @return  object
     * @version 2.0
     */
    public function chat_envato_response( $chat_id )
    {
        $data['where'] = ['chat_id' => $chat_id];
        $data['table'] = 'chats_envato_responses';
        
        return $this->get_one( $data );
    }
    
    /**
     * Add Chat Envato Response
     *
     * @param   array $data
     * @return  integer
     * @version 2.0
     */
    public function add_chat_envato_response( $data )
    {
        return $this->add( $data, 'chats_envato_responses' );
    }
    
    /**
     * Delete Chat Envato Response
     *
     * @param   integer $chat_id
     * @return  boolean
     * @version 2.0
     */
    public function delete_chat_envato_response( $chat_id )
    {
        $data['where']['chat_id'] = $chat_id;
        $data['table'] = 'chats_envato_responses';
     ...
     * @param   integer        $id
     * @param   boolean|string $update_time
     * @return  boolean
     * @version 1.4
     */
    ...
    public function tickets( array $options = [] )
    {
        ...
        {
						if ( empty( $options['my_departments'] ) )
						{
        ...
        if ( ! empty( $options['department_id'] ) )
        {
            $data['where']['t.department_id'] = $options['department_id'];
				...
				if ( ! empty( $options['my_departments'] ) )
        {
            $data['where_in'] = ['column' => 't.department_id', 'values' => $this->departments_ids_mine()];
        }
        
        if ( ! empty( $options['my_departments'] ) )
        {
            $ids = $this->departments_ids_mine();
            $ids = ( ! empty( $ids ) ) ? $ids : [0];
            
            $data['where_in'] = ['column' => 't.department_id', 'values' => $ids];
        }
