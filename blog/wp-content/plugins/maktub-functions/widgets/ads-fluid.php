...
if ( ! class_exists( 'epcl_ads_fluid' ) ) {
	...
	class epcl_ads_fluid extends WP_Widget{
		...
		function __construct(){
			$widget_ops = array('description' => esc_html_x('Display a fluid ads. Note: the max ads width is 300px and height is unlimited.', 'admin', 'maktub'));
			parent::__construct( false, esc_html_x('(EP) Fluid Ads', 'admin', 'maktub'), $widget_ops);
		}
		...
		function form($instance){
			...
			<p>
				<label for="<?php echo $this->get_field_id('width'); ?>"><?php echo esc_html_x( 'Width of advertise to show:', 'admin', 'maktub'); ?></label>
				<input id="<?php echo $this->get_field_id('width'); ?>" name="<?php echo $this->get_field_name('width'); ?>" type="text" value="<?php echo $width; ?>" size="3" />
				<br />
				<small><?php echo esc_html_x( 'Only integer values are allowed:', 'admin', 'maktub'); ?></small>
			</p>
			<p>
				<label for="<?php echo $this->get_field_id('align'); ?>"><?php echo esc_html_x('Align:', 'admin', 'maktub') ?> </label>
			...
			<p>
				<label for="<?php echo $this->get_field_id('ads'); ?>"><?php echo esc_html_x('Ads Code:', 'admin', 'maktub') ?> </label>
