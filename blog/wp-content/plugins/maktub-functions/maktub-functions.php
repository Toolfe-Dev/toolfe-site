<?php
/**
 * Plugin Name: Maktub Theme Functions
 * Plugin URI: https://1.envato.market/ep-portfolio-themes
 * Description: This plugin enables core functions for <a href="https://1.envato.market/wp-maktub-preview">Maktub - Minimal & Lightweight Theme for WordPress</a> by <a href="https://1.envato.market/ep-portfolio-themes">EstudioPatagon</a>.
 * Version: 1.3.0
...
defined( 'ABSPATH' ) or die( 'No script kiddies please!' ); // No direct access allowed
 ...
 if ( !class_exists('epcl_maktub_theme_functions') ) {
    ...
    if( !defined('EPCL_THEMESLUG') ) define('EPCL_THEMESLUG', 'maktub'); // Do not change
    if( !defined('EPCL_ITEM_ID') ) define('EPCL_ITEM_ID', '38348402'); // Do no change

    class epcl_maktub_theme_functions {

        public function __construct() {

            /* Check if Maktub Theme is activated */

            if( get_template() != 'maktub' ){
                return;
            }
		...
		public function gallery_shortcode($atts){
			...
			$output .= '<ul class="columns-'.$columns.' grid-container grid-parent np-tablet np-mobile">';
				...
				foreach($images as $image){
					...
					$url = wp_get_attachment_image_src($image->ID, 'large');
		...
		public function posts_columns($defaults){
            $defaults[EPCL_THEMEPREFIX.'_post_image'] = esc_html__('Image', 'maktub');
			return $defaults;
		}
	 ...
	 new epcl_maktub_theme_functions();
