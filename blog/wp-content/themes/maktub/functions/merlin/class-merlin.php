...
class Merlin {
    ...
    protected $child_theme_zip = null;
	/**
	...
	function __construct( $config = array(), $strings = array() ) {
        ...
        $this->child_theme_zip        = $config['child_theme_zip'];
	...
	protected function license() {
		...
		<div class="merlin__content--transition">
            ...
            <?php if ( ! $is_theme_registered ) : ?>
                <div class="merlin__content--license-email">
					<div class="merlin__content--license-email-wrapper">
                        <input type="email" name="email" id="license-email" class="js-license-email" autocomplete="on" autocorrect="off" autocapitalize="off" spellcheck="false" placeholder="Your email">
					</div>
				</div>
				<div class="merlin__content--license-key">
					...
					<div class="merlin__content--license-key-wrapper">
                        <!-- <input type="email" id="license-email" class="js-license-email" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false" placeholder="Your email"><br><br> -->
						<input type="text" id="license-key" class="js-license-key" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false" placeholder="Envato Purchase Code">
                ...
                <div class="merlin__content--license-user">
					<div class="merlin__content--license-user-wrapper">
                        <input type="text" id="license-user" class="js-license-user" autocorrect="off" autocapitalize="off" spellcheck="false" placeholder="Your Envato Username">
                        <label for="license-user"><small>e.g. estudiopatagon <a href="http://prntscr.com/qk7llp" target="_blank">check example</a></small></label>
					</div>
                </div>
                <br>
                <p><a href="https://help.market.envato.com/hc/en-us/articles/202822600-Where-Is-My-Purchase-Code-" target="_blank">How to get my purchase code</a></p>
			<?php endif; ?>
	...
	public function generate_child() {
		...
		if ( ! file_exists( $path ) ) {
            ...
            $zip_from = EPCL_ABSPATH.'/functions/import/'.$this->child_theme_zip;
            $zip_to = get_theme_root();

            unzip_file($zip_from, $zip_to);
	...
	public function _ajax_activate_license() {
        ...
        if ( empty( $_POST['license_user'] ) ) {
			wp_send_json(
				array(
					'success' => false,
					'message' => __( 'Please add your Envato Username this will ensure <b>only you</b> can use the license.', '@@textdomain' ),
				)
			);
		}
        ...
        $license_user = sanitize_text_field( $_POST['license_user'] );

		if ( ! has_filter( 'merlin_ajax_activate_license' ) ) {
			$result = $this->edd_activate_license( $license_key, $license_email, $license_user );
		} else {
			$result = apply_filters( 'merlin_ajax_activate_license', $license_key, $license_email, $license_user );
		}
	 ...
	 * @return array
	 */
	protected function edd_activate_license( $license, $license_email, $license_user ) {
		$success = false;
        ...
        $response = $this->activate_license( $license, $license_user, false, $license_email );
			...
			<?php foreach ( $import_info as $slug => $available ) : ?>
				...
				<li class="merlin__drawer--import-content__list-item status status--Pending" data-content="<?php echo esc_attr( $slug ); ?>">
                    ...
                    <label for="default_content_<?php echo esc_attr( $slug ); ?>">
                        <?php
                        if( $slug == 'csf') $slug = 'Theme Options';
                        if( $slug == 'redux') $slug = 'Redux (Theme Options)';
                        if( $slug == 'after_import') $slug = 'After Import (Menus)';
                        ?>
    ...
    protected function verify_license($license = false, $client = false){
        $res = array();
		if(!empty($license) && !empty($client)){
			$data_array =  array(
				"product_id"  => EPCL_THEMESLUG,
				"license_file" => null,
    ...
    protected function call_api($method, $url, $data){
